---
name: "papergapcheck"
description: "Find where the paper lacks the evidence its argument needs, so the writer never papers over a hole with hand-waving, overclaiming, or a fabricated number. For each gap it either reserves a clearly-labelled placeholder (a hypothesis-marked figure/table that keeps structure and logic stable) or records the gap, and emits a single paper/EXPERIMENT_PLAN.md saying exactly which experiment would fill each gap. A review sub-skill of $paperwrite; also usable standalone. Invoke when the user says \"content gap\", \"what's missing\", \"check for gaps\", \"find weaknesses\", or $papergapcheck."
---
...
Review sub-skill of `$paperwrite`. Numbers come from `results/` (our single source of
truth here — the equivalent of watson's `evidence.json`). This skill is mostly mechanical, so it uses only one field of
...
experiments the `/goal` sequence must produce. On a real run, apply the two responses below.
Synthetic does not waive schema fidelity: the fixture must still contain every
game/model/seed/condition/sensitivity dimension promised by the approved
embedded contract. An aggregate synthetic value cannot fill a separately approved
per-game or per-model artifact.
...
1. **Reserve a slot** (when the structure needs the float to stay coherent). Insert a
   ...
   - a figure generated with **clearly-labelled synthetic data of the expected shape**
     (the `paper/fig/make_figs.py` matplotlib convention paperwrite already uses),
...
2. **Plan the experiment.** Every gap gets an entry in `paper/EXPERIMENT_PLAN.md`
   ...
   `reports/03_EXPERIMENT_PLAN.html`, though a confirmed gap should be echoed back to
   `$runplan`, which will convert them into later `/goal` executions).
...
## Workflow
1. Read the `experiment-plan-contract` JSON embedded in
   `reports/03_EXPERIMENT_PLAN.html` and require `approval_status=approved`. Run
   `python3 .agents/skills/paperwrite/scripts/plan_conformance.py` before the
   semantic audit. Missing/merged artifacts and missing result dimensions are
   gaps even when every surviving number is grounded.
2. Read the compiled paper and every file in `results/`. Build the claim→file map.
3. Walk each section's claims; for each ask "what evidence makes this true, and do we
   have it?" Classify: supported / weakness (disclose) / gap (reserve+plan).
4. For gaps: preserve or restore the embedded contract's unique labeled slot; never merge
   two approved slots into an aggregate substitute. Always add a plan entry.
5. Write `paper/EXPERIMENT_PLAN.md`; report the gap list and which claims are currently
   propped by reserved slots vs fully supported.
6. Re-run both plan conformance and the semantic audit after experiments land:
   a gap closes only when its reserved slot is filled by the required result
   dimensions from new `results/`.
...
Before the gap list is final, **spawn a separate sub-agent** (Codex `spawn_agent` tool) to
