...
Review sub-skill of `$paperwrite`. This
...
**Spawn a fresh sub-agent** (Codex `spawn_agent` tool — keep the reader separate from whoever wrote
...
- A bridging sentence states a *real* logical link; do not paper over a missing
  ...
  content gap — route it to `papergapcheck`, do not write a smooth sentence over it.
