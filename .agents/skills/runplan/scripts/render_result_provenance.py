...
def linkify_values(report: str, result_ids: list[str]) -> str:
...
INTERACTION = r"""
<style>
.result-value{color:inherit;font-weight:800;text-decoration:underline;text-decoration-style:dotted;text-underline-offset:3px;cursor:pointer}
...
def update_report(report: Path, ledger: Path, plan: Path) -> int:
    ...
    source = linkify_values(report.read_text(encoding="utf-8"), list(payload))
