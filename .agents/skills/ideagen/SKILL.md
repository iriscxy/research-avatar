...
4. **Recommendation eligibility is a hard gate, not a ranking preference.** An
   `already exists` or `[UNVERIFIED]` candidate cannot appear in the ranked
   recommendation slate or human pick options. A `differentiable (needs
   framing)` candidate stays visible only in a separate **Needs reforge — not
   recommended** area and cannot become `I1` or the default pick unless the
   researcher explicitly requests incremental/application work. During the
   devil's-advocate pass, ask whether the closest work can absorb the proposed
   change without changing its central causal/mechanistic story; if yes, it is
   not an independent recommendation. When no candidate has a defensible
   `novel` contribution, output an honest **no viable recommended idea** result
   and ask the researcher to redirect the search. Never force a ranked slate
...
Present **up to 4–6 eligible ranked ideas** in `reports/02_IDEA_REPORT.html` in A5 rank order; fewer, including zero, is valid when the evidence does not support more. Use a separate ranked-summary table with the columns `ID | Idea | Novelty status | Closest work | Concrete difference | Strongest objection | Confidence` (and a rightmost `Ethics risk` column only when Step 2 flagged at least one candidate); do not show numeric scores. Only eligible `novel` candidates receive stable pick ids `I1, I2, …`. Put all other surviving candidates in **Needs reforge — not recommended**, without pick IDs. **Each recommended idea — in the table, card, and pick options — leads with the identical mandatory `Plain-language summary` from A3.** The card then gives the technical one-sentence pitch followed by method steps · hypothesis · MVE · own-work overlap · feasibility · risk · contribution type · unresolved novelty evidence. **Stop. ask the user directly for the researcher to pick / kill / redirect** (offer eligible ideas by id + the `Plain-language summary`; if none are eligible, offer redirect only). Never auto-proceed.
