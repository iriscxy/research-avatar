...
5. Keep local-Agent and GPT roles explicit:
   - Use the Agent CLI that owns the session—Codex in a Codex session, otherwise Claude Code—for the always-available executable project chat. Keep reproducible plot authoring, safe layout interpretation, and table editing on the configured local Agent path.
   ...
   - Let the selected project Agent semantically classify each ordinary chat turn from the current message and recent history as `read_only` or `execute`; never select read versus write mode from a hard-coded action-verb list. An explicit deletion, clearing, or overwrite request is itself authorization: execute it immediately within the exact requested scope and never add a second “确认执行” turn. Detect success across both editable source files and researcher-visible PNG/PDF/PPTX/SVG artifacts. Run the Agent in its own process group and terminate the whole group on timeout so a reported failure cannot continue mutating files in the background.
