...
2. Treat each researcher-visible transition as a state machine. Distinguish pending, running, candidate-ready, composed, and approved states; never infer readiness merely from a file left on disk.
   ...
   - Generated prose must be pdflatex-safe before it becomes an editable candidate: escape prose `%`, `&`, `#`, and `_`, put mathematics inside LaTeX math delimiters, and replace Unicode math glyphs with LaTeX commands. Correct hazards once in the same GPT conversation, then fail with an actionable preflight error before compilation if any remain. Apply the same preflight to direct edits of accepted prose. The preflight must also balance-check math delimiters themselves (`$`, `\(`/`\)`, `\[`/`\]`) — masking correctly-paired math before the specials check leaves a stray/unclosed delimiter untouched, which reaches pdflatex as "Missing $ inserted." and crashes the whole document (a real batch-writing run hit this at paragraph 8/19; the fatal error was only found because `compile_paper()` surfaces `!`-prefixed pdflatex lines instead of a tail of the raw log).
...
5. Keep local-Agent and GPT roles explicit:
   - Use the configured local Agent (Codex CLI, or Claude Code when that owns the session) for reproducible plot authoring, safe layout interpretation, and table editing (`/api/table/agent-edit`, per-figure panel generation, mechanism figure reconstruction). There is intentionally no general-purpose "always-available executable project chat" launcher/dialog anywhere in the UI: it was removed (not merely hidden) after a researcher reported it silently failing to make a requested edit visible, and diagnosis found the deeper, separately-fixed bug (module-level FIGURES/TABLES definitions never reloading after the Agent edited paper_studio.json on disk) rather than a fixable chat-specific defect worth the surface area of a whole free-form conversational entry point. Do not reintroduce a global chat launcher/dialog/`/api/agent-chat` endpoint; route any future local-Agent-driven capability through its own narrow, purpose-built control (a specific button tied to a specific artifact/action) instead.
   ...
   - Any local-Agent action that edits `paper/paper_studio.json` (figure/table structural add/remove) must reload the in-memory FIGURES/FIGURE_ORDER/TABLES/TABLE_ORDER definitions in place afterward (`reload_figure_and_table_definitions_if_paper_studio_json_changed`); these are module-level globals loaded once at process startup and otherwise silently go stale until a manual restart, which looks indistinguishable from the edit never having happened.
