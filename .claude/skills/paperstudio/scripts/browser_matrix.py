...
class Matrix:
    ...
    def approved_caption_update(self) -> None:
        ...
        assert page.locator("#figure-approve").is_disabled()
        assert page.locator("#figure-approve").inner_text() == "已插入正文"
        page.locator("#figure-caption").fill(revised_caption)
        assert page.locator("#figure-caption-save").is_enabled()
        assert page.locator("#figure-approve").is_enabled()
        assert page.locator("#figure-approve").inner_text() == "更新 Caption → PDF"
        page.locator("#figure-approve").click()
        page.wait_for_timeout(100)
        assert page.locator("#figure-approve").inner_text() == "已插入正文", {
            "label": page.locator("#figure-approve").inner_text(),
    ...
    def failure_restores_visible_drafts(self) -> None:
        ...
        figure = next(iter(fixture.get("figures", [])), None)
        if figure:
            figure.update(
                ready=True,
                insertion_ready=True,
