...
This is a review sub-skill of `/paper-write`. It reads the same inputs
(`aris-profile/PROFILE_AUTO.md` via `$ARIS_PROFILE`, `outputs/03_EXPERIMENT_PLAN.html`,
`results/`) and writes into `paper/`. **Converse in the language the user is using
(Chinese here); write the theory statements themselves in the paper's language
(English by default).**
...
`$ARIS_PROFILE/fulltext/txt/` (e.g. the CRH / representation-geometry line). Reuse
...
**When invoked by `/paper-write`, use the passed personalization context, not an
independent pick:** ground the spine in the SAME `reference_paper` paper-write chose as
...
paper-write's shared `anti_self_plagiarism` pass so a theorem's prose is not a near-copy of
one of her prior papers. Standalone (`/paper-theorization`), fall back to reading the
profile yourself.
