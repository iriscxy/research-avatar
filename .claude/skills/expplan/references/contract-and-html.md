...
- **One canonical numbering registry:** before rendering, collect every Projected
  ...
  repair numbers independently in HTML. Run the application-owned target-outline
  identifier allocator before rendering, then use the experiment-plan validator
  as a final assertion. A duplicate returned by the writing model must be
  resolved before persistence rather than surfaced as a user-facing plan error.
