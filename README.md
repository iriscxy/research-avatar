...
整套东西只有 **5 个 skill + 5 个工具 + 1 份画像**,没有任何框架依赖。
...
> 这份画像由 `/profile-construct` 生成,其余四个 skill 只**读**它,不重复挖掘。缺失或过时就重跑 `/profile-construct`。
...
| 2 | `/ideagen` | 文献综述 → 三透镜想 idea(方法优先)→ 对照自己工作 + 并行工作查新 → 排序 idea 榜(`outputs/01_IDEA_REPORT.html`) | **选一个 idea** |
| 3 | `/workplan` | 从选定 idea → claim 驱动的 `outputs/02_EXPERIMENT_PLAN.html`(从投影摘要倒推)+ 可选论文骨架 | 批准 plan |
| 4 | `/run-plan` | 用 `/goal` 执行 plan,跑到关口停 | 每个结果块审"支不支持 claim" |
| 5 | `/paper-write` | 个性化写论文(套风格 / 自引 / 反自抄) | 逐节审稿 |
...
### `/workplan` — 写实验计划
- 从你选的 idea 出发,**先读最接近的几篇论文全文**扎根 baseline/数据集/指标 → 从投影摘要**倒推**出 claim 驱动的 `outputs/02_EXPERIMENT_PLAN.html`(对照实验一次只动一个变量;可选嵌入论文骨架)→ **你批准**。
...
├── 01_IDEA_REPORT.html         /ideagen 的 idea 榜
├── 02_EXPERIMENT_PLAN.html     /workplan 的实验计划
├── 03_PAPER_PLAN.html          (可选)可视化论文骨架
├── 04_EXPERIMENT_TRACKER.html  /run-plan 的实验追踪
└── 05_FINDINGS.html            结果汇总
```

每份都配一个 `.zh.html` 中文镜像(母语级、非逐字机翻,第二人称"你/你的")。
...
5. **匹配你的栈,不匹配你的超参** —— 代码贴你环境(Experiment Templates);lr/batch/seed 由 `02_EXPERIMENT_PLAN.html` 定(任务决定)。
...
│   └── skills/                5 个 skill(+ scholar-translation-zh 翻译规范)
...
本助手保留了前者(7 条纪律),把后者**交给了你本人**(人审,不引入第二个模型当裁判)。其余的全部砍掉——所以它只有 5 个 skill,而不是几十个。
