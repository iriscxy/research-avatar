#!/usr/bin/env python3
"""Check that a paper and result bundle preserve an approved experiment plan.
        ...
        for artifact in contract.get("paper_artifacts", []):
            ...
            if actual is None:
                ...
                if set(map(str, visible_dimensions)) != set(map(str, artifact.get("dimensions", []))):
                    ...
                    violations.append({"issue": "artifact_visible_dimension_contract_drift",
                                       "id": artifact.get("id")})
                # LaTeX float source can expose captions and table cells, but
                # vector/Python figure axes and legends live inside the
                # included PDF.  The Studio configuration is the deterministic
                # rendering contract for that visible surface, so include its
                # dimensions, data-grid labels, and panel semantics instead of
                # incorrectly requiring every axis word in the LaTeX caption.
                configured_surface = ""
                if isinstance(configured, dict):
                    configured_surface = json.dumps(
                        {
                            "dimensions": configured.get("visible_dimensions", []),
                            "columns": configured_table_columns(configured),
                            "panels": configured.get("panels", []),
                        },
                        ensure_ascii=False,
                    )
                normalized_block = normalized_names(
                    [actual.get("block", "") + configured_surface]
                )[0]
                missing_visible = [
            ...
            if not isinstance(configured, dict):
                ...
                if expected_columns:
                    ...
                    normalized_actual = normalized_names(actual_columns)
                    normalized_expected = normalized_names(expected_columns)
                    # Contract table shells describe metric columns; the
                    # records grid additionally needs one leading row-label
                    # column (usually Method/Setting). Do not misclassify that
                    # necessary identifier as an extra experimental metric.
                    if (
                        normalized_actual != normalized_expected
                        and not (
                            len(normalized_actual) == len(normalized_expected) + 1
                            and normalized_actual[1:] == normalized_expected
                        )
                    ):
                        artifact_result["ok"] = False
                        violations.append(
