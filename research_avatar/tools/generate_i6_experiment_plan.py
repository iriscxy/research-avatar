...
def metric_record(metric: tuple[str, str, str, str, str, str, str, str], claims: list[str]) -> dict:
    ...
    return {
        ...
        "construct": construct,
        "claim_mappings": [{"claim_id": claim, "measurement_role": "DIRECT" if provenance != "ADAPTED" else "PROXY", "cannot_establish": "This metric alone cannot establish a causal conflict.", "companion_requirements": ["paired intervention", "gradient trace", "noise calibration"]} for claim in claims],
...
def rebuild_contract(soup: BeautifulSoup, cell_map: dict[str, list[str]]) -> None:
    ...
    contract.update({
        ...
        "dataset_citations": [
            {"name": "MultiWOZ 2.2", "url": "https://aclanthology.org/2020.nlp4convai-1.13/", "role": "headline task-oriented dialogue benchmark"},
            {"name": "Schema-Guided Dialogue", "url": "https://ojs.aaai.org/index.php/AAAI/article/view/6394", "role": "cross-domain and unseen-service robustness benchmark"},
        ],
    ...
    claim_specs = [
        ...
        ("C2", "CRCD recovers the direction and magnitude of controlled cross-objective behavioral effects while preserving task utility.", "Effect error is not lower than controls or task success degrades beyond the fixed non-inferiority margin.", ["M-ACE", "M-TS"]),
        ...
        ("C5", "Noise calibration and paired behavioral evidence are necessary to prevent false conflict declarations.", "Removing either component does not materially worsen false positives or human agreement.", ["M-CF1", "M-FPR", "M-NAT"]),
    ]
    contract["claims"] = []
    ...
    contract["metric_contract"] = [metric_record(metric, [claim[0] for claim in claim_specs]) for metric in METRICS]
    ...
    contract["consistency_requirements"] = {"canonical_terms": [b[0] for b in BASELINES] + [m[0] for m in METRICS], "source_values": ["D-I6"], "formal_links": [c[0] for c in claim_specs]}
